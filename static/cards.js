...
var MAX_FLIP = 26;
var deck;
var fname = new Array(2);
var lname = new Array(2);
var player_matches = [0,0];
var player_turn=0;
var player_mode;
...
function singlePlayerMode(selected_card)
{
  if(card_flips <= MAX_FLIP-1)
  {
    ...
    if(found_matches.length==deck_size)
    {
      $("#victory-song")[0].load();
      $("#victory-song")[0].play();
      $('#player-win-msg').text('You have won! Do you want to play again?');
...
function twoPlayerMode(selected_card)
{
  // if the card was not seleceted before then add it to selected list
  if($(selected_card.lastElementChild).hasClass('hidden')== false)
  {
    ...
    if((match_index =isMatched(selected_card, cards_selected))!=-1)
    {
      ...
      //count number of matches to the respective player
    ...
    if(found_matches.length==deck_size)
    {
      $("#victory-song")[0].load();
      $("#victory-song")[0].play();
      $('#player-win-msg').text('Player '+(player_matches[0]>player_matches[1]? 1: 2)+' have won! Do you want to play again?');
      $('#win-modal').modal('setting', 'closable', false).modal('show');
...
function switchPlayerTurn()
{
  ...
  $('#player-turn-msg').text('Its now player '+(player_turn+1)+" turn.")
...
function memory_game (selected_card) {
  ...
  {
    singlePlayerMode(selected_card);
  }
  else
  {
    twoPlayerMode(selected_card);
  }
}
function updatePlayerInfo()
{
  if(player_mode=="single-player")
  {
    ...
    $('#player1_name').text(fname[0]+" "+lname[0]);
  }
  else
  {
    $("#player1-matches").text(player_matches[0]);
    $("#player2-matches").text(player_matches[1]);
    $('#player1_name').text(fname[0]+" "+lname[0]);
    $('#player2_name').text(fname[1]+" "+lname[1]);
  }
}
function playAgain()
{
